...
erpc_status_t QSPIService_service::handleInvocation(uint32_t methodId, uint32_t sequence, Codec * codec, MessageBufferFactory *messageFactory)
{
    ...
    switch (methodId)
    {
        ...
        case kQSPIService_changeFrequency_id:
        {
            erpcStatus = changeFrequency_shim(codec, messageFactory, sequence);
            break;
        }
...
// Server shim for changeFrequency of QSPIService interface.
erpc_status_t QSPIService_service::changeFrequency_shim(Codec * codec, MessageBufferFactory *messageFactory, uint32_t sequence)
{
    erpc_status_t err = kErpcStatus_Success;
    ...
    if (err == kErpcStatus_Success)
    {
        ...
        nestingDetection = true;
#endif
        result = changeFrequency(prescaler);
#if ERPC_NESTED_CALLS_DETECTION
        nestingDetection = false;
#endif
    ...
    if (err == kErpcStatus_Success)
    {
        ...
        // Build response message.
        codec->startWriteMessage(kReplyMessage, kQSPIService_service_id, kQSPIService_changeFrequency_id, sequence);
